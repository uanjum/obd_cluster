...
class GUIApp:
    ...
    def build_gui(self):
        ...
        for j in range(0,2):
            for i in range(0,3):
                ...
                self.value_list.append(self.master.create_text(140+i*260, 200+j*200, text = 0, fill = 'White', font = TextFont, anchor=S))
    ...
    def update_values(self):
        if (str(self.select_button['state']) == "normal"):
            ...
            convert = [1]*6
            for i in range(0,6):
                res = self.connection.send_command(self.labels(i))
                if res is not None:
                    convert[i] = -(((res.value.magnitude-minVal[obdCommandDescription.index(self.labels[i])]) /
                               (maxVal[obdCommandDescription.index(self.labels[i])]-minVal[obdCommandDescription.index(self.labels[i])])*180))
                    
                    self.master.itemconfig(self.arc_list[i], extent = convert[i])
                    self.master.itemconfig(self.value_list, text = int(res.value.magnitude))
        
        self.master.after(10, self.update_values())


    def run_GUI(self):
        self.connection = OBDConnection()
        self.connection.look_for_connections()
        self.connection.connect_to_port(0)
